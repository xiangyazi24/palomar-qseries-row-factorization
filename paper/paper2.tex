...
weight-one transformation formulas; the five characteristic components in
the $S$-image reduce algebraically to the two displayed components.  One boundary
correction vanishes by an odd-theta pairing.  For the other boundary we give an index-four
...
\]
Writing $a'_m=b+(0,m/5)$, this five-term expression reduces to
\[
  \widehat F(-1/\tau)
  =\frac{\tau}{2\sqrt5}\left[
    (1+e^{-\pi i/5})\vartheta_{a'_1,-a}^{c_2,c_1}(\tau)
    +(1+e^{3\pi i/5})\vartheta_{a'_4,-a}^{c_2,c_1}(\tau)
  \right].
\]
...
holomorphic.  Hence $\widehat F$ is not a weight-one harmonic Maass
form.  The displayed $S$- and $T$-identities do not by themselves
specify a closed vector-valued representation.
\end{theorem}
...
There is a further reduction not needed for the transformation formula.
For $a'_m=b+(0,m/5)$, the characteristic $a'_3=(1/2,1/2)$ is odd
relative to $-a$, so
$\vartheta_{a'_3,-a}^{c_2,c_1}=0$.  Corollary~2.9(1)--(3), applied to
integer translation, dual translation, and simultaneous negation of
the characteristics, also gives
\[
  \vartheta_{a'_0,-a}^{c_2,c_1}
    =e^{-\pi i/5}\vartheta_{a'_1,-a}^{c_2,c_1},
  \qquad
  \vartheta_{a'_2,-a}^{c_2,c_1}
    =e^{3\pi i/5}\vartheta_{a'_4,-a}^{c_2,c_1}.
\]
Substitution yields the displayed two-component form.
\end{proof}
...
kernel evaluation and depends only on Lean's usual three axioms.  It
also checks the full Laurent-series row-model factorization: the two
Jacobi legs, their reindexing, the formal Jacobi triple product, the
$H$-level convolution, even support, and the halved-variable identity
in Theorem~\ref{thm:mk-factorization}.  On the analytic side, Lean
defines Zwegers' normalized error kernel, proves its derivative and
boundary limits, constructs the convergent general-characteristic
indefinite theta and unary theta components, and proves the analytic
odd-theta cancellation used for the transverse $c_1$ factor.  It
formalizes the complete specialized proof of Zwegers' Lemma~2.8,
including differentiation under the integral, integration by parts,
the two Gaussian evaluations and the constant-by-oddness argument.
It also proves genuine two-dimensional Poisson summation by
periodization on the two-torus, specializes it to the completed kernel,
and derives Corollary~2.9(5), \eqref{eq:S-transform}, and its two-term
reduction.  The $T$-law, the exact $\partial_{\overline\tau}$ and
$\xi_1$ formulas, and the explicit nonharmonicity witness are likewise
kernel-checked.  The more interpretive assertion that this explicit
sum is the canonical completion of its holomorphic part, and a closed
vector-valued representation containing the full characteristic
orbit, are not formalized and are not claimed here.
...
The coefficient theorem \texttt{mk\_factorization}, in namespace
\texttt{Ch10} within \texttt{QseriesFormalization}, proves the finite
convolution for \texttt{MKcoeff}.  It is paired with
\texttt{coneDiffH\_two\_mul}, which identifies the even cone
coefficients with $B_N$, and \texttt{coneDiffH\_odd}, which proves
odd-level vanishing.  The Laurent-series assembly is
\texttt{paper2MKLaurent\_eq\_jacobi\_sq\_mul\_cone}; the injective
even-exponent compression concludes with
\texttt{paper2RowLaurent\_eq\_jacobi\_sq\_mul\_B}, which is exactly
the boxed formula in Theorem~\ref{thm:mk-factorization}.  No
source-faithfulness theorem in the repository currently identifies
\texttt{MKcoeff} with the older
Chapter--10 object
\[
