...
analytic completed theta explicitly. Comparator checks nine declarations:
...
5. `zwegers_lemma28`: the specialized Fourier-transform identity, including
   its exact square-root constant and exponential sign;
6. `completedTheta_add_one`: the weight-one translation law;
7. `completedTheta_S`: both the five-characteristic inversion law and its
   algebraic reduction to the two displayed components;
8. `exact_differential_image`: the exact Wirtinger derivative and
   Bruinier--Funke image as mixed unary-theta sums;
9. `completedTheta_not_harmonic`: explicit nonvanishing at `2i`, hence failure
...
- `Solution.lean`: transports all nine statements to the extracted proof
  closure.
- `QseriesFormalization/`: exact 39-file substantive source closure.
- `AxiomAudit.lean`: permanent `#print axioms` checks for the nine declarations.
